...
class SwitchReset(hass.Hass):
    ...
    def set_switches(self, kwargs):
        ...
        # If we don't know about it initialize, if we do set the switch
        # appropriately
            ...
            if type == "input_boolean" or type == "input_select" or type == "input_slider" or type == "device_tracker":
                if entity in self.device_db:
                    if self.device_db[entity] != state[entity]["state"]:
                        self.log_notify("Setting {} to {} (was {})".format(
                            entity, self.device_db[entity], state[entity]["state"]))
                        new_state = self.set_state(
                            entity, state=self.device_db[entity])
                else:
                    self.log_notify("Adding {}, setting value to current state ({})".format(
